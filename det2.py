...
def hsv_filter(image,  min_val_w, max_val_w):
    """
    ...
    img_filtered = cv2.bitwise_and(image, image, mask=mask_white)
...
region_of_interest_points = np.array([[0,720],[0,600], [600,240], [720,240], [1280,600],[1280,720]])

#defining color thresholds
min_val_y = np.array([15,80,190])
max_val_y = np.array([30,255,255])
min_val_w = np.array([0,0,180])
max_val_w = np.array([255, 70, 255])
...
while True:
    ...
    if ret:
        ...
        bilateral = cv2.bilateralFilter(frame, 9, 80, 80)
        # cv2.imshow('bilateral', bilateral)
        
        hsv = hsv_filter(bilateral,  min_val_w, max_val_w)
        # cv2.imshow('hsv', hsv)
        
        canny = cv2.Canny(hsv, 150, 255)
        #cv2.imshow('canny', canny)
        
        cropped = region_of_interest(canny, np.array([region_of_interest_points], np.int32))
        cv2.imshow('cropped', cropped)
        
        hough, lines = hough_transform(frame, canny, 100, discard_horizontal = 0.4)
